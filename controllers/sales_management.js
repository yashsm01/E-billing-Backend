 ...
 */
const salesManagementController = {
  list: async (req, res) => {
    try {
      if (!req.tableUid) {
  ...
  add: async (req, res) => {
    try {
      ...
      if (req.body.save_as_draft == false) { // If True stock details and payment does not update
        ...
        await controllers.commonController.addOrUpdateretailCustomerPayment(retailCustomerMasterSchema, req.retailerId, req.retailOutletId, data, req.tableUid);
        //-----------------------------------------------------------
      }
      return res.status(200).send({ success: 1, message: "Sales added successfully", salesId: salesId });
    } catch (error) {
      console.error(error);
      controllers.logger.error(req, error.message);
      return res.status(500).send({
    ...
    try {
      ...
      if (req.body.save_as_draft == false) {
        ...
        if (draftedSalesOrder) {
          await retailCustomerMasterSchema.update(// 1 : credit // 0 : cash
            {
              payment_amount: Sequelize.literal(`payment_amount + ${draftedSalesOrder.total_amount}`),
              pending_amount: Sequelize.literal(`pending_amount + ${[0, 2, 3].includes(salseOrderInfo.payment_type) ? 0 : (draftedSalesOrder.total_amount)}`),
              pending_count: Sequelize.literal(`pending_count + ${[0, 2, 3].includes(salseOrderInfo.payment_type) ? 0 : 1}`),
          ...
          for (let element of req.body.items) {
            ...
            //---------------  steps start for Update sales-order-details --------------------------------------------------------------//
              ...
              if (req.body.save_as_draft == false) {
                ...
                let qtyChange = (-1) * (Number(element.qty) - Number(existingSalesOrderDetails.qty)); // Assuming existingPurchaseOrderDetails.qty was fetched earlier
      ...
      if (req.body.save_as_draft == false) {
        ...
        if ([0, 2, 3].includes(req.body.payment_type)) {
          await retailCustomerMasterSchema.update(// 1 : credit // 0 : cash
            {
              payment_amount: Sequelize.literal(`payment_amount + ${[0, 2, 3].includes(salseOrderInfo.payment_type) ? updatedTotalAmount : updatedTotalAmount}`),
              pending_amount: Sequelize.literal(`pending_amount + ${[0, 2, 3].includes(salseOrderInfo.payment_type) ? 0 : (-1) * (salseOrderInfo.total_amount)}`),
              pending_count: Sequelize.literal(`pending_count + ${[0, 2, 3].includes(salseOrderInfo.payment_type) ? 0 : -1}`),
            },
            { where: { consumer_id: req.body.customer, retailer_id: req.retailerId, retail_outlet_id: req.retailOutletId } }
          );
          ...
          await retailCustomerMasterSchema.update(// 1 : credit // 0 : cash
            {
              payment_amount: Sequelize.literal(`payment_amount + ${[0, 2, 3].includes(salseOrderInfo.payment_type) ? updatedTotalAmount : updatedTotalAmount}`),
              pending_amount: Sequelize.literal(`pending_amount + ${[0, 2, 3].includes(salseOrderInfo.payment_type) ? totalDetails.total_amount : updatedTotalAmount}`),
              pending_count: Sequelize.literal(`pending_count + ${[0, 2, 3].includes(salseOrderInfo.payment_type) ? 1 : 0}`),
            },
            { where: { consumer_id: req.body.customer, retailer_id: req.retailerId, retail_outlet_id: req.retailOutletId } }
          );
        }
      ...
      return res.status(200).send({ success: 1, message: "Sales Updated Successfully", salesId: req.body.sales_id });
  ...
  QrCodeDetail: async (req, res) => {
    try {
      ...
      let salesModel = await models.salesSchema(TUID);

      let salesData = await salesModel.salesModels.findOne({
        where: {
          // id: req.params.id,
          // retailer_id: req.retailerId,
          // retail_outlet_id: req.retailOutletId
      ...
      let salesDetails = await salesModel.salesDetailModels.findAll({
        where: {
          ...
          retailer_id: salesData.retailer_id,
          retail_outlet_id: salesData.retail_outlet_id
        },
        include: [
          {
            model: models.productsModel,
            attributes: ['id', 'name', 'category', 'uom', 'unit_size', 'content', 'image'],
            as: 'products',
            raw: true,
            nest: true
          },
    ...
    try {
      ...
      await Promise.all(req.body.items.map(async (element) => {
        let data = {
          ...
          total_margin: totalDetails.total_margin,
          // Group 3 -- payment
          payment_status: false, // Drafts should not be marked as paid
        }
        salesList.push(data);
      }));
      await salesSchema.create({
      ...
      return res.status(200).send({ success: 1, message: "Draft saved successfully", salesId: salesId });
    } catch (error) {
      console.error(error);
      controllers.logger.error(req, error.message);
      return res.status(500).send({
...
module.exports = salesManagementController;
