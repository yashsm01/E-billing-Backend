...
const purchaseOrderController = {
  list: async (req, res) => {
    try {
      ...
      const purcheseSchema = await models.puchaseSchema(req.tableUid);

      let purchaseOrderlist = await purcheseSchema.purchaseOrderModels.findAll({
        where: whereClause,
        include: [
          {
          ...
          {
            model: purcheseSchema.bankDetailsModel,
            as: 'bank_details',
            raw: true,
            nest: true
          }
  ...
  add: async (req, res) => {
    try {
      ...
      await purchaseOrderSchema.create({
        ...
        validation_no: req.body.validation_no,
      });
      ...
      if (req.body.save_as_draft == false) { // If True stock details and payment does not update
        ...
        await controllers.commonController.addOrUpdateretailDistributorPayment(retailDistributorSchema, req.retailerId, req.retailOutletId, data, req.tableUid);
        //-----------------------------------------------------------
      }
      return res.status(200).send({ success: 1, message: "Purchase bill added successfully", poId: poId });
    } catch (error) {
      console.error(error);
      controllers.logger.error(req, error.message);
      return res.status(500).send({
    ...
    try {
      ...
      if (req.body.save_as_draft == false) {
        ...
        if (draftedPurchaseOrder) {
          await retailDistributorSchema.update(// 1 : credit // 0 : cash
            {
              payment_amount: Sequelize.literal(`payment_amount + ${draftedPurchaseOrder.total_amount}`),
              pending_amount: Sequelize.literal(`pending_amount + ${[0, 2, 3].includes(PurchaseOrderInfo.payment_type) ? 0 : (draftedPurchaseOrder.total_amount)}`),
              pending_count: Sequelize.literal(`pending_count + ${[0, 2, 3].includes(PurchaseOrderInfo.payment_type) ? 0 : 1}`),
          ...
          for (let element of req.body.items) {
            ...
            //>>>>>>>>>>>>>>>>NEW ADDED/UPDATED Purchase Details <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<//
              ...
              if (req.body.save_as_draft == false) {
                ...
                let qtyChange = Number(element.qty) - Number(existingPurchaseOrderDetails.qty); // Assuming existingPurchaseOrderDetails.qty was fetched earlier
                let unitQtyChange = Number(element.unit_qty) - Number(existingPurchaseOrderDetails.unit_qty);
                ...
                if (qtyChange < 0) {
                  ...
                  await productStockSchema.update(
                    {
                      qty: Sequelize.literal(`qty + ${Math.abs(qtyChange)}`),
                      unit_qty: Sequelize.literal(`unit_qty + ${unitQtyChange}`)
                    },
                    { where: { product_id: element.product_id, retailer_id: req.retailerId, retail_outlet_id: req.retailOutletId } }
                  );
                ...
                // ------------------------------ Update Stock----------------------------------
                let freeChange = element.free - existingPurchaseOrderDetails.free_qty; // Assuming existingPurchaseOrderDetails.free_qty was fetched earlier

                //------------------------------ Handle free quantity increase---------------------------------
                if (freeChange > 0) {
                  console.log('Free increased by:', freeChange);

                  // Update Product Stock (increase)
                  await productStockSchema.update(
                    {
                      free_stock: Sequelize.literal(`free_stock + ${freeChange}`),
                    },
                    { where: { product_id: element.product_id, retailer_id: req.retailerId, retail_outlet_id: req.retailOutletId } }
                  );
                ...
                //----------------- Handle free quantity decrease------------------------------------------------
                if (freeChange < 0) {
                  console.log('Quantity decreased by:', Math.abs(freeChange));

                  // Update Product Stock (decrease)
                  await productStockSchema.update(
                    { free_stock: Sequelize.literal(`free_stock + ${Math.abs(freeChange)}`) },
                    { where: { product_id: element.product_id, retailer_id: req.retailerId, retail_outlet_id: req.retailOutletId } }
                  );
                ...
                //----------------------------- Handle no change in free quantity--------------------------------------------
                if (freeChange === 0) {
                  console.log('No change in quantity.');
                  // No update to stock needed
                }
            ...
            await productStockSchema.update(
              {
                ...
                free_stock: Sequelize.literal(`free_stock - ${Math.abs(item.free)}`)
              },
      ...
      if (req.body.save_as_draft == false) {
        ...
        if ([0, 2, 3].includes(req.body.payment_type)) {
          await retailDistributorSchema.update(// 1 : credit // 0 : cash
            {
              payment_amount: Sequelize.literal(`payment_amount + ${[0, 2, 3].includes(PurchaseOrderInfo.payment_type) ? updatedTotalAmount : updatedTotalAmount}`),
              pending_amount: Sequelize.literal(`pending_amount + ${[0, 2, 3].includes(PurchaseOrderInfo.payment_type) ? 0 : (-1) * (PurchaseOrderInfo.total_amount)}`),
              pending_count: Sequelize.literal(`pending_count + ${[0, 2, 3].includes(PurchaseOrderInfo.payment_type) ? 0 : -1}`),
            },
            { where: { distributor_id: req.body.distributor_id, retailer_id: req.retailerId, retail_outlet_id: req.retailOutletId } }
          );
          ...
          await retailDistributorSchema.update(// 1 : credit // 0 : cash
            {
              payment_amount: Sequelize.literal(`payment_amount + ${[0, 2, 3].includes(PurchaseOrderInfo.payment_type) ? updatedTotalAmount : updatedTotalAmount}`),
              pending_amount: Sequelize.literal(`pending_amount + ${[0, 2, 3].includes(PurchaseOrderInfo.payment_type) ? totalDetails.total_amount : updatedTotalAmount}`),
              pending_count: Sequelize.literal(`pending_count + ${[0, 2, 3].includes(PurchaseOrderInfo.payment_type) ? 1 : 0}`),
            },
            { where: { distributor_id: req.body.distributor_id, retailer_id: req.retailerId, retail_outlet_id: req.retailOutletId } }
          );
        }
      ...
      //---------------------------------
      return res.status(200).send({ success: 1, message: "Purchase Order Updated Successfully" });
